...
import { RecService, VeznikService } from '../../../services/reci/';
...
export class VeznikComponent implements OnInit, AfterViewInit {
  ...
  saveAvailable() {
    ...
    if (this.tokenStorageService.isVolunteer() && this.veznik.vlasnikID === 3)
      return true;
  ...
  removeWord(): void {
    this.recService.remove(this.id, 8).subscribe({
      next: (data) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Успех',
